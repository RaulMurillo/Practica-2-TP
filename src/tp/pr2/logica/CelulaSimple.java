...
/**
 * Esta clase representa una celula simple del mundo. Contiene
 * atributos privados para contabilizar el numero de pasos en
 * los que la celula no se ha movido y el numero de pasos dados
 *(tanto si se ha movido como si no) realizados en el
 * mundo.
 * 
...
public class CelulaSimple extends Celula {
	...
	public final int PASOS_REPRODUCCION = 4; 
	...
	/**
	 * Incrementa el numero de pasos que ha dado una celula
	 ...
	 * 
	 * @return true si ha llegado al maximo permitido y la 
	 * celula debe morir
	 */
	...
	/**
	 * Realiza el movimiento de una celula simple colocada en
	 *  la posición (f,c).
	 * 
	 * @return la casilla a la que se ha movido la celula o null
	 *  en caso de que no se mueva la celula.
	 */
	public Casilla ejecutaMovimiento(int f, int c, Superficie superficie){
		...
		if (destino == null) { // No se puede mover
			...
			if (incPasDad()) {
				...
				System.out.println("Muere la celula de la casilla " +
				origen + " por no reproducirse");
			}
			// No le toca reproducirse. Aumentan sus pasos no dados.
			else if (incPasNoMov()) { // Decide si debe morir
				superficie.eliminarCelula(origen);
				System.out.println("Muere la celula de la casilla " +
				origen + " por falta de actividad.");
			}
