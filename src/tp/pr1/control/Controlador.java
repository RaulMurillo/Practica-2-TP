...
/**
 * Esta clase contiene el interprete de los
 * posibles comandos que se pueden ejecutar en consola.
...
public class Controlador {
	 ...
	 * @param mundo sobre el que se ejecutaran los comandos.
	 * @param in Scanner para realizar las operaciones de lectura.
	 */
	...
	/**
	 * Consiste en bucle en el que se pide un
	 * comando al usuario y se ejecuta dicho comando.
	 */
	public void realizarSimulacion(){
		...
		do{
			 ...
			 * (http://www.forosdelweb.com/f13/validar-palabra-sin-importar-mayuscula-minuscula-511492/)
			 */
			...
			if(comando.equals(PASO)){
				...
				int x = in.nextInt();
				int y = in.nextInt();
				if(!mundo.crearCelula(x, y))System.out.println("Imposible crear celula, posición no válida.");
				else System.out.println("Creamos nueva celula en la posición: (" + x + "," + y + ")");
			}
			else if(comando.equals(ELIMINARCELULA)){
				int x = in.nextInt();
				int y = in.nextInt();
				if(!mundo.eliminarCelula(x, y))System.out.println("Imposible eliminar celula, posición no válida.");
				else System.out.println("Se ha eliminado la célula de la posición: (" + x + "," + y + ")");
			}
			...
			else if(!comando.equals(SALIR)){	// Si el comando es incorrecto se muestra un mensaje de error.
				System.out.println("Comando no válido.");
			}
		}while(!comando.equals(SALIR));	// El bucle termina cuando el usuario teclea el comando SALIR.
